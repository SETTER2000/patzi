...
// Введите глобальное `before` для наших тестов Mocha.
before((done)=>{
  try {
    ...
    // Взять синглтон Sails.
    // (Он будет выставлен как глобальный на мгновение, при условии, что это приложение не имеет
    // настроил это поведение.)
    ...
    // Обратите внимание, что мы смешиваем в env vars, параметрах CLI и файле .sailsrc, используя
    // метод `.getRc ()`, если возможно. Но также обратите внимание, что мы смешиваем в
    // несколько дополнительных переопределений, чтобы убрать помехи из тестового вывода, обеспечить
    // мы работаем с чистой базой данных и т. д.
    ...
    configOverrides = Object.assign({}, configOverrides, {
      ...
      policies: Object.assign({}, configOverrides.policies, {
        ...
        // Укажем дыру в любой глобальной политике, чтобы убедиться, что тестовый бегун может
        // фактически получить доступ к токену CSRF.
      ...
      datastores: Object.assign({}, configOverrides.datastores, {
        default: Object.assign({}, (configOverrides.datastores||{}).default, {
          ...
          // Чтобы тесты работали с локальной базой данных mysql, например,
          // добавьте конфигурацию здесь: (например, раскомментируйте две строки ниже)
          // адаптер: 'sails-mysql',
          // url: 'mysql: //root@127.0.0.1: 3306 / pba',
        })
      }),
