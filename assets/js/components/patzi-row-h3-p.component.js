/**
 * <patzi-row-h3-p>
 * -----------------------------------------------------------------------------
 * Начальный пример возможного построения компонента.
 *
 * @type {Component}
 *
 * @event click   [emitted when clicked]
 * -----------------------------------------------------------------------------
 */

parasails.registerComponent('patziRowH3P', {
  //  ╔═╗╦═╗╔═╗╔═╗╔═╗
  //  ╠═╝╠╦╝║ ║╠═╝╚═╗
  //  ╩  ╩╚═╚═╝╩  ╚═╝
  props: [
    'styleObj',
    'objData'
  ],

  //  ╦╔╗╔╦╔╦╗╦╔═╗╦    ╔═╗╔╦╗╔═╗╔╦╗╔═╗
  //  ║║║║║ ║ ║╠═╣║    ╚═╗ ║ ╠═╣ ║ ║╣
  //  ╩╝╚╝╩ ╩ ╩╩ ╩╩═╝  ╚═╝ ╩ ╩ ╩ ╩ ╚═╝
  data: function () {
    return {
      //...
    };
  },

  //  ╦ ╦╔╦╗╔╦╗╦
  //  ╠═╣ ║ ║║║║
  //  ╩ ╩ ╩ ╩ ╩╩═╝template
  template: `
<!-- Start Sample Area -->
  <section class="sample-text-area">
    <div class="container">
      <h3 class="">{{objData.preferredLocale === 'ru' ? objData.h3.ru : objData.h3.en}}</h3>
<!--      <h3 class="text-heading">{{objData.preferredLocale === 'ru' ? objData.h3.ru : objData.h3.en}}</h3>-->
      <p class="sample-text">
      {{objData.preferredLocale === 'ru' ? objData.textUnderHeading.ru : objData.textUnderHeading.en}}
      </p>
      <!--<p><button @click="sayHello()">Подключиться к сокету</button></p>-->
      <el-divider><i class="el-icon-star-on"></i></el-divider>
    </div>
  </section>
  <!-- End Sample Area -->
`,


  //  ╦  ╦╔═╗╔═╗╔═╗╦ ╦╔═╗╦  ╔═╗
  //  ║  ║╠╣ ║╣ ║  ╚╦╝║  ║  ║╣
  //  ╩═╝╩╚  ╚═╝╚═╝ ╩ ╚═╝╩═╝╚═╝
  beforeMount: function () {
    //...
  },


  mounted: async function () {
    if (this.objData.h3 === undefined) {
      throw new Error('Neither `:d3-data`  was passed in to <patzi-row-h3-p>, but one or the other must be provided.');
    }
  },

  beforeDestroy: function () {
    //...
  },

  //  ╦╔╗╔╔╦╗╔═╗╦═╗╔═╗╔═╗╔╦╗╦╔═╗╔╗╔╔═╗
  //  ║║║║ ║ ║╣ ╠╦╝╠═╣║   ║ ║║ ║║║║╚═╗
  //  ╩╝╚╝ ╩ ╚═╝╩╚═╩ ╩╚═╝ ╩ ╩╚═╝╝╚╝╚═╝
  methods: {
   //...
  }
});
